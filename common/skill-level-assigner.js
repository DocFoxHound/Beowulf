...
async function updateSkillOnMemberChange(oldMember, newMember) {
  try {
    ...
    const oldHighest = Math.max(Number(oldPrestige?.raptor_level || 0) || 0, Number(oldPrestige?.raider_level || 0) || 0);
    const newHighest = Math.max(Number(newPrestige?.raptor_level || 0) || 0, Number(newPrestige?.raider_level || 0) || 0);
