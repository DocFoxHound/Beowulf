...
const { componentItemToEntity } = require('../common/entities/items-to-entities');
const { upsertGameEntities } = require('../common/game-entities-sync');

const MAX_FILE_BYTES = Number(process.env.COMPONENT_ITEM_UPLOAD_MAX_FILE_BYTES || 2_000_000);
const MAX_ROWS = Number(process.env.COMPONENT_ITEM_UPLOAD_MAX_ROWS || 1000);
const EXISTING_FETCH_LIMIT = Number(process.env.COMPONENT_ITEM_UPLOAD_EXISTING_LIMIT || 20000);
const SUPPORTED_EXTS = new Set(['.csv']);
const SUPPORTED_MIME = new Set(['text/csv']);

const name = 'component-item-upload';
...
const data = new SlashCommandBuilder()
  .setName(name)
  .setDescription('Upload ship component definitions (CSV only) into items_components')
  .setDMPermission(false)
  .addAttachmentOption((opt) =>
    opt.setName('file')
      .setDescription('CSV file with name plus any other stat columns')
      .setRequired(true)
  )
...
function resolveRoleList(isLive) {
  const candidates = isLive
    ? [process.env.COMPONENT_ITEM_UPLOAD_ROLE_IDS, process.env.PLAYER_ITEM_UPLOAD_ROLE_IDS, process.env.ENTITY_UPLOAD_ROLE_IDS, process.env.DOC_INGEST_ROLE_IDS]
    : [process.env.TEST_COMPONENT_ITEM_UPLOAD_ROLE_IDS, process.env.TEST_PLAYER_ITEM_UPLOAD_ROLE_IDS, process.env.TEST_ENTITY_UPLOAD_ROLE_IDS, process.env.TEST_DOC_INGEST_ROLE_IDS];
...
function normalizeId(value) {
  if (typeof ItemsComponentsModel.normalizeId === 'function') {
    return ItemsComponentsModel.normalizeId(value);
  }
...
async function loadExistingComponentsByName() {
  try {
    const rows = await ItemsComponentsModel.list({ limit: EXISTING_FETCH_LIMIT, order: 'updated_at.desc' });
    const map = new Map();
    ...
    console.error('[ComponentItemUpload] existing lookup failed:', error?.message || error);
    return new Map();
  }
}
...
async function upsertItems(items, { dryRun = false } = {}) {
  ...
  const existingByName = await loadExistingComponentsByName();
  ...
  for (const item of items) {
    ...
    try {
      ...
      if (!existing && hadUploadId) {
        existing = await ItemsComponentsModel.getById(item.id);
      }
      if (item.id === undefined) {
        item.id = generateItemId();
      }
      ...
      if (dryRun) {
        ...
        const result = await ItemsComponentsModel.update(item.id, patch);
        if (result?.ok) {
          updated += 1;
        ...
        const result = await ItemsComponentsModel.create(item);
        if (result?.ok) {
          created += 1;
          allowEntitySync = true;
      ...
      if (allowEntitySync) {
        const entity = componentItemToEntity(item);
      ...
      console.error('[ComponentItemUpload] upsert error:', error?.message || error);
    }
  }
  let entities = null;
  if (entityCandidates.length) {
    try {
      entities = await upsertGameEntities(entityCandidates, { defaultSource: 'component-item-upload', dryRun });
    } catch (error) {
      errors.push(`Failed to sync entity index: ${error?.message || error}`);
      console.error('[ComponentItemUpload] entity sync failed:', error?.message || error);
    }
  }
...
async function execute(interaction) {
  ...
  if (!memberHasAllowedRole(interaction)) {
    await interaction.reply({ content: 'You do not have permission to upload component items.', ephemeral: true });
    return;
  }
  ...
  try {
    ...
    console.error('[ComponentItemUpload] download failed:', error?.message || error);
  ...
  if (!items.length) {
    await interaction.editReply('No valid component item rows were found in the file.');
    return;
  }
  ...
  let response = `Processed ${summary.total} component item${summary.total === 1 ? '' : 's'} (source rows: ${totalSourceRows}).`;
