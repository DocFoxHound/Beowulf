...
async function createClass(newClass) {
    ...
    const apiUrl = `${process.env.SERVER_URL}/api/class/`; 
    try {
...
async function getClassById(classId){
    ...
    try {
        const response = await axios.get(`${apiUrl}/api/class/${classId}`);
...
async function editClass(classId, updatedUserData) {
    const apiUrl = `${process.env.SERVER_URL}/api/class/${classId}`; // Assuming this is the correct endpoint
    try {
