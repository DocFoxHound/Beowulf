...
function toApiPayload(input = {}) {
  ...
  if (input.manufacturer !== undefined) payload.manufacturer = normalizeString(input.manufacturer);
  if (input.role !== undefined) payload.role = normalizeString(input.role);
  return payload;
}
...
const ShipListModel = {
  ...
  async list(params = {}) {
    const rows = await api.listShipList(params);
    if (!Array.isArray(rows)) return [];
    return rows.map(fromApiRow).filter(Boolean);
  },

  async getById(id) {
    const row = await api.getShipListById(id);
  ...
  async create(doc) {
    ...
    const created = await api.createShipListEntry(value);
  ...
  async update(id, patch) {
    ...
    if (!ok) return { ok: false, errors };
    const updated = await api.updateShipListEntry(id, value);
    return updated ? { ok: true, data: fromApiRow(updated) || updated } : { ok: false, errors: ['update failed'] };
  },

  async remove(id) {
    return !!(await api.deleteShipListEntry(id));
  },
};

module.exports = { ShipListModel };
