/* Model for uex_outposts wrapping `/outposts/` */
...
const STR_FIELDS=['name','nickname','pad_types','star_system_name','planet_name','orbit_name','moon_name','faction_name','jurisdiction_name'];
...
const UexOutpostsModel={
  table:'uex_outposts', validate,toApiPayload,fromApiRow,
  async list(){ const rows=await uex.getAllOutposts(); if(!Array.isArray(rows)) return []; return rows.map(fromApiRow).filter(Boolean); },
  async getById(id){ const row=await uex.getOutpostById(id); return row?fromApiRow(row):null; },
  async upsert(doc){ const {ok,errors,value}=validate(doc); if(!ok) return {ok:false,errors}; try{ await uex.createOrUpdateOutpost(value); return {ok:true}; } catch(e){ return {ok:false, errors:[e?.response?.data||e?.message||'upsert failed']}; } },
};

module.exports={ UexOutpostsModel };
