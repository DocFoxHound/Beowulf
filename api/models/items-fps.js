...
const ItemsFpsModel = {
  ...
  async list(params = {}) {
    const rows = await api.listItemsFps(params);
    if (!Array.isArray(rows)) return [];
    return rows.map(fromApiRow).filter(Boolean);
  },

  async getById(id) {
    const row = await api.getItemsFpsById(id);
  ...
  async create(doc) {
    ...
    const created = await api.createItemsFps(value);
  ...
  async update(id, patch) {
    ...
    if (!ok) return { ok: false, errors };
    const updated = await api.updateItemsFps(id, value);
    return updated ? { ok: true, data: fromApiRow(updated) || updated } : { ok: false, errors: ['update failed'] };
  },

  async remove(id) {
    return !!(await api.deleteItemsFps(id));
  },
};

module.exports = { ItemsFpsModel };
