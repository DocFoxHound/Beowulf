/* Model for uex_commodities_summary wrapping `/summarizedcommodities/` */
...
const UexCommoditiesSummaryModel={
  table:'uex_commodities_summary', validate,toApiPayload,fromApiRow,
  async list(){ const rows=await uex.getAllSummarizedCommodities(); if(!Array.isArray(rows)) return []; return rows.map(fromApiRow).filter(Boolean); },
  async getById(id){ const row=await uex.getSummarizedCommodityById(id); return row?fromApiRow(row):null; },
  async upsert(doc){ const {ok,errors,value}=validate(doc); if(!ok) return {ok:false,errors}; try{ await uex.createOrUpdateSummarizedCommodity(value); return {ok:true}; } catch(e){ return {ok:false, errors:[e?.response?.data||e?.message||'upsert failed']}; } },
};

module.exports={ UexCommoditiesSummaryModel };
