...
async function handleRequiresAction(message, run, client, preloadedDbTables, openai, isAutoGenerated, threadPair) { 
    console.log("Requires Action");
    if(isAutoGenerated === true){ //if this message originated from a user
        const toolCall = run.required_action.submit_tool_outputs.tool_calls[0];
        const contentText = await functionHandler.executeFunction(run, message, preloadedDbTables, openai, client, threadPair);
