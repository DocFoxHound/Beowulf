...
async function handleRequiresAction(message, run, client, jsonData, openai, isAutoGenerated, threadPair) { 
    console.log("Requires Action");
    if(isAutoGenerated === true){ //if this message originated from a user
        const toolCall = run.required_action.submit_tool_outputs.tool_calls[0];
        const contentText = await functionHandler.executeFunction(run, message, jsonData, openai, client, threadPair);
        ...
        const toolCall = run.required_action.submit_tool_outputs.tool_calls[0];
        const contentText = await functionHandler.executeFunction(run, message, jsonData, openai, client);
        run = await addResultsToRun.addResultsToRun(contentText, openai, run.thread_id, toolCall.id, run.id);
        // let messages = await client.beta.threads.messages.list(thread.id);
        let messages = await openai.beta.threads.messages.list(run.thread_id);
